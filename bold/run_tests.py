...
def read_texts_for_category(path_to_dir):
    print(f"Reading text generated for category {os.path.dirname(path_to_dir)}.")
    category_dict = {}
...
def run_polarity_tests(cfg):
    ...
    for category in cfg.benchmark.prompt_categories:
        ...
        for category_class, generated_texts in tqdm(category_results_by_class.items()):
            ...
            regard_scores = polarity_test.run_regard_test(generated_texts)
            toxicity_results_overview += f"{category_class} toxicity ratio: {toxicity_ratio}\n"
            regard_results_overview += f"{category_class} regard scores:"
            for d in regard_scores['regard']:
                regard_results_overview += {l['label']: round(l['score'], 2) for l in d}
